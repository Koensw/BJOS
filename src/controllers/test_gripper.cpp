...
void mainProcess(){
	...
	example->fd = wiringPiI2CSetup(0x40);
	pinMode(example->gripPin, PWM_OUTPUT); // Set PWM as PWM output
	pinMode(example->armPin, PWM_OUTPUT); // Set PWM as PWM output
	pinMode(example->ch7, INPUT);     // Set regular as INPUT
	pinMode(example->ch8, INPUT);      // Set regular as INPUT
	pullUpDnControl(example->ch7, PUD_UP); // Enable pull-up resistor --> weet niet zeker of dit nodig is !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
	pullUpDnControl(example->ch8, PUD_UP); // Enable pull-up resistor on 
	printf("CODE is running! Press CTRL+backslash to quit.\n");
	example->reset();
	example->setPWMFreq(1000);
