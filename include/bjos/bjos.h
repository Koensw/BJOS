...
public:
    /* Init and finalize method should be called by the loader at begin and end
     * WARNING: this should be called only once before starting an instance and after using the last instance else error WILL occur
     */
    static void init();
    static void finalize();
    
    /* Install signal handler that will set a flag that we are not running anymore */
    static void (*installSignalHandler())(int);
    bool isRunning();
...
private:  
    ...
    //TODO: switch to throwing exception instead? maybe better static
